...
import { View, Text, TouchableOpacity, TextInput, Image, Animated, SafeAreaView, LayoutAnimation, UIManager, Alert, Platform, ScrollView, ToastAndroid, ActivityIndicator } from "react-native";
       ...
       .then((response) => response.json())
        .then((result) => {
          if(result.statusCode == 200){
            setLoading(false)
            // console.log("task detail",result?.data?.assignTaskList?.[0]?.commentData?.employeData)
          ...
          loading?(
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1 }}>
                <View style={{ width:"100%", flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginVertical:10, paddingBottom:10 }} >
                  <Text style={{ flex:6, fontSize:14, fontWeight:"600", color:Style.headerBgColor }}>Task Details</Text>
                   <View style={{ width:"100%", flex:4, backgroundColor:taskSumry.status =='Task_Stop'?'#E51E1E':taskSumry.status =='Completed'?'#85BD2A':taskSumry.status =='Assigned'?'#1AA4FF':'#eb984e', height:30, justifyContent:'center', alignItems:'center', borderRadius:5 }} >
                      ...
                      taskSumry.assignTaskList[0].commentData.map((item, index) => (
                        <View key={index} style={{ width:'100%', backgroundColor:Style.inputBgColor, borderRadius:10, elevation:1, padding:10, marginBottom:10 }}>
                          <View style={{ flexDirection:'row', justifyContent:'space-between' }}>
                            {
                              taskSumry.assignTaskList[0].employeData.map((emp, ind)=>{
                                return(
                                  <Text key={ind} style={{ fontSize:14, fontFamily:'Poppins-Medium', color:Style.secondryTextColor, paddingBottom:5 }}>
                                    {emp?.fullName || "Unknown"}
                                  </Text>
                                )
                              })
                            }
                            <Text style={{ fontSize:14, fontFamily:'Poppins-Medium',color: taskSumry?.status === 'Task_Stop' ? '#E51E1E' : taskSumry?.status === 'Completed' ? '#85BD2A' : taskSumry?.status === 'Assigned' ? '#1AA4FF' : '#eb984e' }}>
                              { taskSumry?.status?.replace(/_/g, ' ')
                                .toLowerCase()
                                .replace(/\b\w/g, char => char.toUpperCase())}</Text>
                          </View>
                
                          <Text style={{ fontSize:14, fontFamily:'Poppins-Medium', color:Style.placeHolderTextColor }}>{item.message} </Text>
                          ...
                          <View style={{ flexDirection:'row', justifyContent:'space-between', paddingTop:5 }}>
                            ...
                            <Text numberOfLines={1} ellipsizeMode='tail' style={{ flex:1, fontSize:12, fontFamily:'Poppins-Medium', color:Style.placeHolderTextColor }}>
                              Assigned to: {item.creatorData?.fullName || "Unknown"}
                            </Text>
                          </View>
                        </View>
                      ))
                  ...
                  <View>
                    <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center' }} >
                      ...
                      <View style={{ width:'100%', height:40, borderRadius:5, backgroundColor:Style.inputBgColor, elevation:1, marginBottom:10, padding:5, elevation:4 }}>
                         <TextInput value={review} onChangeText={value=> setReview(value)} placeholder="No Review" placeholderTextColor="#999" style={{ flex:1, backgroundColor:'#eee', borderRadius:5, padding:5, color:"#074173", fontFamily:'Poppins-Mediumkk' }} />
                      </View>
